...
class NetoperatorREV:
    """The class of an operator aim to maximize revenue"""

    def __init__(self, logger, network: Network, version, method, grain_p, grain_alpha):
        self.logger = logger
        ...
        self.p_list_all = np.linspace(0, 1, num=grain_p + 1, endpoint=True)  # todo: extend to epsilon=0
        self.alpha_list_all = np.linspace(0, 1, num=grain_alpha + 1, endpoint=True)  # todo: extend to epsilon=0
    ...
    def load_budget2scheme2result(self):
        """Load the latest result pkl under ../eplots_rev .
        ...
        result_pkl_path_list = [d for d in os.listdir(f"../eplots") if f"{self.network.did}_{self.method}" in d and 'rev.pkl' in d]
    ...
    def dump_budget2scheme2result(self):
        """Dump the result pkl with cur_time under ../eplots_rev .

        :return: None
        :rtype: None
        """
        nowTime = datetime.datetime.now().strftime("%m%d%H%M%S")
        result_pkl_path = f"../eplots/{self.network.did}_{self.method}_{nowTime}_rev.pkl"
    ...
    def get_M_brute(self, budget, tilderR, tilderS):
        ...
        if len(tilderS) <= budget:
            incr_R = self.compute_incr(tilderS, tilderR)
            return tilderS, incr_R

        combs = itertools.combinations(tilderS, budget)
        result = {}
        for i, group in enumerate(combs):
            incr_R = self.compute_incr(group, tilderR)
            result[group] = incr_R
...
#########################################################################################################################
    ...
    #     """Get supplier with largest marginal, 方法名字带下划线的都是有记录 requester2IncrMlast 更有效率但是可读性差
    #
    ...
    # def get_pq_list2(self, grain_p, grain_alpha): # 弃用，corner case 的而处理太啰嗦了
