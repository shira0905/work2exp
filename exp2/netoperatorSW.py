...
class NetoperatorSW:
    """The class of an operator aim to maximize social welfare"""
    ...
    def load_budget2scheme2result(self):
        """Load the latest result pkl under ../eplots .
        ...
        result_pkl_path_list = [d for d in os.listdir(f"../eplots") if f"{self.network.did}_{self.method}" in d and 'sw.pkl' in d]
    ...
    def search_budget2scheme2result(self, budget, tilderR, tilderS):
        ...
        :param tilderS:
        :type tilderS:
        :return: result=(suppliers M, social welfare sw, meaningless revenue) in dictionary budget2scheme2result with legal key
        :rtype: (set(), int, float)
        """
    ...
    def dump_budget2scheme2result(self):
        """Dump the result pkl with cur_time under ../eplots .
        ...
        result_pkl_path = f"../eplots/{self.network.did}_{self.method}_{nowTime}_sw.pkl"
        pickle.dump(self.budget2scheme2result, open(result_pkl_path, 'wb'))


    def get_marginal_greedy(self, tilderR, tilderS, M_last, sw_last, budget_last):
        """Get the result (M, sw)_{b+1} if based on (M_last, sw_last)_b
        ...
        :param budget_last:
        :type budget_last:
        :return: M_last = M_last+{u_star}, sw = social welfare of M_last+{u_star}
        :rtype:
        """
        if len(M_last) < budget_last:  # S不够
            return M_last, sw_last
    ...
    def get_M_brute(self, budget, tilderR, tilderS):
        """Get ground truth opt participating suppliers star_M by brute-force method and sw V(star_M)
